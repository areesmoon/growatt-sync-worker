...
async function runCorrection() {
  try {
    ...
    if (firstSnapshot.empty) {
      console.log("❌ Tidak ada data awal ditemukan di collection:", FIRESTORE_COLLECTION);
      return;
    }
    ...
    console.log(`📄 Titik Awal Ref  [ID: ${firstId}] : ${firstData.timestamp}`);
    console.log(`   └─ Charge: ${firstData.system?.chargeTotal} kWh | Discharge: ${firstData.system?.dischargeTotal || 0} kWh | Master Ah: ${firstData.master?.ah}Ah | Slave Ah: ${firstData.slave?.ah}Ah`);

    console.log(`📄 Titik Akhir     [ID: ${lastId}] : ${lastData.timestamp}`);
    console.log(`   └─ Charge: ${lastData.system?.chargeTotal} kWh | Discharge: ${lastData.system?.dischargeTotal || 0} kWh | Master Ah: ${lastData.master?.ah}Ah`);

    // 3. Hitung Delta Energi Kumulatif dari Awal ke Akhir (Melibatkan Charge & Discharge)
    ...
    console.log(`📊 Akumulasi Energi: ΔCharge = +${deltaChargeKwh.toFixed(3)} kWh | ΔDischarge = -${deltaDischargeKwh.toFixed(3)} kWh | Netto = ${netEnergyKwh.toFixed(3)} kWh`);
